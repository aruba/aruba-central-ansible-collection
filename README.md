HPE Aruba Networking Central Classic Ansible Collection
=========

This deprecated version of the HPE Aruba Networking Central Ansible Network collection provides a set of platform dependent configuration management modules and plugins specifically designed for Classic Aruba Central, which is a cloud-based Network Management System.
...
* Minimum supported Classic Aruba Central version is 2.5.2
...
- The plugins in this collection use Classic Aruba Central's REST API. For information on REST API and how to obtain an access for using REST API, visit the HPE Networking Developer Hub: [Getting Started with REST API](https://developer.arubanetworks.com/aruba-central/docs/api-getting-started)
- An API token must be created for a user on Classic Aruba Central's API Gateway and a valid, non-expired **`access_token`** must be used. For more information on how to get started with the API Gateway you can also watch this [YouTube Video](https://www.youtube.com/watch?v=tWEsL7zSOB0).
...
- Classic Aruba Central also provides a token refresh mechanicsm for renewing an expired access token. You can use the [Refresh Token API](https://developer.arubanetworks.com/aruba-central/reference/xxxxxx) to do this and use a valid access token in your inventory/host file.


## Inventory/Host File 
### [](https://github.com/aruba/aruba-cnetral-ansible-collection#inventory-variables)Inventory Variables

The variables that should be defined in your inventory for your Classic Aruba Central account are:

- `ansible_host`: Cluster-specific Base-URL for the API Gateway on Classic Aruba Central in FQDN format, which can be found in the API Documentation URL on API Gateway
...
- `ansible_httpapi_central_access_token`: Classic Aruba Central's API access token
...
### Including Classic Central collection
...
At HPE Networking we're dedicated to ensuring the quality of our products, so if you find any
